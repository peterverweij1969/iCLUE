...
/**
 * probabilities affecting land use allocation:
 * 1. ease of change - static over time (specific per land use)
 * 2. neighbourhood  - static over time (specific per land use)
 * 3. suitability    - might change over time (based on parameter maps. specific per land use)
 * 4. demandWeight   - changes in iterations and over time (difference available and demand. specific per land use)
 * 
...
public class TheModel {
    ...
    private static final String ERROR_PARAMETERS = "Cannot run model. Parameters are incomplete: "+ParameterStatus.NEW_LINE+"%s";
    ...
    public TheModel(Parameters params){
    ...
    /**
     * land use suitabilities can be determined using a variety of methods, 
     * such as:
    ...
    public Projections createLanduseProjections() {
        ...
        try {
            ...
            for (Clazz administrativeUnit: params.getAdministrativeUnits().getDatakind().getClasses()) {
                ...
                for (int year=params.getBaseline().getYear()+1; year<= params.getTargetTime(); year++) {
                    ...
                    LanduseRasterData previousLanduseAndAge = projections.createRasterData(year-1);// new LanduseRasterData(previousLanduseMap, previousAgeMap);
        ...
        SpatialDataset allLanduseProjections = mergeProjectionsFromAllAdminUnits(landuseProjectionsForAllAdminUnits); // Melanie states: Dinamica 'fuzzifies' at the admin borders
        SpatialDataset allAgeProjections = mergeProjectionsFromAllAdminUnits(ageProjectionsForAllAdminUnits); // Melanie states: Dinamica 'fuzzifies' at the admin borders
    ...
    private LanduseRasterData allocate(int year, LanduseRasterData previousLanduseRasterData, DemandValidators demands) {
        ...
        AllocationHelper helper = new AllocationHelper(params.getLanduses(), params.getDrivers(), suitabilityCalculator, params.getConversions());
        ...
        while ((!IterationResultEvaluator.IterationResult.ALLOCATION_ACCEPTABLE.equals(iterationResult))
                && (!IterationResultEvaluator.IterationResult.MAX_NUMBER_OF_ITERATIONS_REACHED.equals(iterationResult))){
            // create new landcover output
            allocationMaps.clearOutputs();
            resultingLanduseMap = allocationMaps.addOutput(true); int LANDUSE_RESULT_INDEX = 0;
            ...
            switch (iterationResult) {
                ...
                case DIFFERENCE_TOO_SMALL_IN_MOVING_AVERAGE: {
                    ...
                    allocationMaps = helper.createAllocationMapsWithStochasticShock(evaluator.getDeviationStatusPerLanduse(resultingLanduseMap)); // apply stochasticity to suitabilities
    ...
    private RasterData deriveAgeMap(RasterData previousAgeMap, RasterData previousLanduseMap, RasterData resultingLanduseMap) {
        RasterDataStack rasterStack = RasterDataFactory.createStack(); 
        rasterStack.addInput(previousAgeMap);      int INDEX_PREVIOUS_AGE = 0;
        rasterStack.addInput(previousLanduseMap);  int INDEX_PREVIOUS_LANDUSE = 1;
        rasterStack.addInput(resultingLanduseMap); int INDEX_RESULTING_LANDUSE = 2;
        ...
        for (CellStack cellStack: rasterStack.getCellCursor()) {
            ...
            else if (cellStack.inputValues[INDEX_PREVIOUS_LANDUSE].intValue() != cellStack.inputValues[INDEX_RESULTING_LANDUSE].intValue())
                rasterStack.setOutputValues(cellStack, new Number[] {1});
            else
                rasterStack.setOutputValues(cellStack, new Number[] {cellStack.inputValues[INDEX_PREVIOUS_AGE].intValue() + 1});
        }
        return result;
    }
    ...
    private void logIterationResult(IterationResult iterationResult, int year, RasterData landuseMapFromIteration, DemandValidators validators, IterationResultEvaluator evaluator) {
        ...
        if (iterationIsSuccessfull) {
            ...
            if (shockCount > 0)
                Log.log(Level.INFO, 
                    String.format("year %d, #iterations %3d (%d)", year, iterationCount, shockCount),
                    null, LOG_TOKEN);
            else
                Log.log(Level.INFO,
                    String.format("year %d, #iterations %3d", year, iterationCount),
                    null, LOG_TOKEN);
        } else {
            allocationProgressPanel.addIteration(year, evaluator.getIterationCount(), landuseMapFromIteration, validators);
        }
    }
    ...
    private static class LanduseAndAgeDataset {
        private final SpatialDataset landuseData;
        private final SpatialDataset ageData;
        ...
        public LanduseAndAgeDataset(SpatialDataset landuseBaseline, Landuses landuses) {
            ...
            RasterData ageMap = LanduseRasterData.createIntialAgeMap(landuseBaseline.getRasterData(), landuses);
            this.ageData = new SpatialDataset();
            this.ageData.setCaption("Age");
            this.ageData.add(ageMap, year);
        }
        
        public static LanduseAndAgeDataset createByAdministrativeBoundaryCut(
                SpatialDataset landuseBaseline, 
                Landuses landuses, 
                SpatialDataset administrativeUnits, 
                Category administrativeUnitOfInterest) {
            return new LanduseAndAgeDataset(landuseBaseline.cut(administrativeUnits, administrativeUnitOfInterest), landuses);
        }
