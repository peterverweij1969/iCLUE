...
public class LanduseDistributions extends ArrayList<LanduseDistribution>{
    ...
    public List<Integer> getSortedYears(Clazz administrativeUnit) {
        ...
        for (LanduseDistribution dist: this) {
            if ((administrativeUnit.equals(dist.getAdministrativeUnit()))
                    && (!years.contains(dist.getYear())))
    ...
    public Map<Landuse, Integer> getAreaAmounts(Clazz administrativeUnit, int year) {
        ...
        if (years.contains(year)) {
            ...
            return getInterpolatedAreaAmounts(
                    administrativeUnit, 
                    getPreviousDefinedYear(years, year),
                    year,
    ...
    public boolean setAreaAmount(Clazz administrativeUnit, int year, Landuse landuse, int area) {
        for (LanduseDistribution distribution: this) {
            if ((administrativeUnit.equals(distribution.getAdministrativeUnit()))
                && (year == distribution.getYear())
    ...
    private Map<Landuse, Integer> getInterpolatedAreaAmounts(Clazz administrativeUnit, 
            int previousDefinedyear,
            int year,
            int nextDefinedYear) {
        // Linear interpolation is implemented here (might use enum with various interpolation methods)
        Map<Landuse, Integer> result = new HashMap<>();
        ...
        for (Entry<Landuse, Integer> previousEntry: previous.entrySet()) {
            ...
            if ((previousArea == null) || (nextArea == null))
                throw new RuntimeException(String.format(ERROR_INTERPOLATE_AREA, lu.getCaption(), year, previousArea, previousDefinedyear, nextArea, nextDefinedYear));
    ...
    private void redistributeRoundErrors(Map<Landuse, Integer> result, int amountToRedistribute, Map<Landuse, Integer> differenceAreaAmount) {
        // TODO: really redistribute instead of putting into the landuse which changes the most area
        Landuse lu = null;
        int difArea = 0;
...
public static class LanduseDistribution {
    ...
    public LanduseDistribution setAdministrativeUnit(Clazz administrativeUnit) {
        ...
        public String toString() {
            return getAdministrativeUnit().getCaption() +", "+getYear() +" -> "+ getLanduse().getCaption() +" of "+ getArea() +" cells";
        }
